                ...
                .flatMap(person -> {
                    ...
                    return clientRepository.findByPersonId(person.getId())
                            .flatMap(client -> {
                                ...
                                if (client.getStatus() == com.polytechnique.tiibntick.domain.model.enums.client.ClientStatus.SUSPENDED) {
                                    return Mono.error(new InvalidCredentialsException("Votre compte a été suspendu"));
                                }
                                if (client.getStatus() == com.polytechnique.tiibntick.domain.model.enums.client.ClientStatus.REVOKED) {
                            ...
                            .switchIfEmpty(deliveryPersonRepository.findByPersonId(person.getId())
                                    .flatMap(deliveryPerson -> {
                                        ...
                                        if (deliveryPerson
                                                .getStatus() == com.polytechnique.tiibntick.domain.model.enums.deliveryPerson.DeliveryPersonStatus.SUSPENDED) {
                                            return Mono
                                                    .error(new InvalidCredentialsException("Votre compte a été suspendu"));
                                        }
                                        if (deliveryPerson
                                                .getStatus() == com.polytechnique.tiibntick.domain.model.enums.deliveryPerson.DeliveryPersonStatus.REVOKED) {
                                            return Mono
                                                    .error(new InvalidCredentialsException("Votre compte a été bloqué"));
                                        }
                                        if (deliveryPerson
                                                .getStatus() == com.polytechnique.tiibntick.domain.model.enums.deliveryPerson.DeliveryPersonStatus.REJECTED) {
                                         ...
                                         * if (deliveryPerson
                                         * .getStatus() ==
                                         * com.polytechnique.tiibntick.domain.model.enums.deliveryPerson.DeliveryPersonStatus.
                                         * PENDING) {
                                         * return Mono.error(new InvalidCredentialsException(
