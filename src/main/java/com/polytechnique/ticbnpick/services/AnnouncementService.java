...
public class AnnouncementService {
            ...
            return addressRepository.save(pickup).flatMap(savedPickup -> {
                ...
                return addressRepository.save(delivery).flatMap(savedDelivery -> {
                    ...
                    return announcementRepository.save(announcement)
                            .map(savedAnnouncement -> mapToResponse(savedAnnouncement, savedPickup, savedDelivery, savedPacket));
                });
            });
        });
    }
    ...
    private Mono<AnnouncementResponseDTO> populateDetails(Announcement announcement) {
        ...
        Mono<Packet> packetMono = announcement.getPacketId() != null ? 
                packetRepository.findById(announcement.getPacketId()) : Mono.just(new Packet());
    ...
    private AnnouncementResponseDTO mapToResponse(Announcement announcement, Address pickup, Address delivery, Packet packet) {
        ...
        return announcementRepository.findById(id)
                .flatMap(announcement -> {
                    ...
                    return announcementRepository.save(announcement).flatMap(savedAnnouncement -> {
                        ...
                        Mono<Address> deliveryUpdate = addressRepository.findById(savedAnnouncement.getDeliveryAddressId())
                                .flatMap(address -> {
                        ...
                        return Mono.zip(packetUpdate, pickupUpdate, deliveryUpdate)
                                .map(tuple -> mapToResponse(savedAnnouncement, tuple.getT2(), tuple.getT3(), tuple.getT1()));
                    });
                });
    }
